...
def detect_eve(Data, VSD_model, VSC_3Cmodel, wrtrD_3C, wrtrC_3C, cfg):
    ...
    if len(Onsets) > 0:   # ---------------------- Check if there are detected events
...
# ================================================================ Models
hm_models="./models/"
# --------------------------------------------- Load pre-trained VSDdeep model for Piton de la Fournaise Volcano
VSD_model = tf.keras.models.load_model(hm_models + 'VSDdeep_PF.h5',
                                       custom_objects={'SeqSelfAttention':SeqSelfAttention,
                                       'FeedForward':FeedForward,
                                       'LayerNormalization':LayerNormalization})
# --------------------------------------------- Load pre-trained VSCdeep model for Piton de la Fournaise Volcano
...
cfg.wind_steep = 20.0   # Step to overlap [s]
cfg.hm_out = './output_DC/'
cfg.t_ini = UTCDateTime("2019-02-01T00:00:00.00")
cfg.t_fin = UTCDateTime("2019-03-01T00:00:00.00")
cfg.t_delta = 60*60*1  # delta to process in s
# -------------------------------------- Output control for Classificatiton
cfg.VSDdeep_thsld = 0.5 
cfg.minLastEve_for_class = 5   # Minimum last to compute the FFT for classification
...
while t1 < t2:
    ...
    flOUT_trigs_3C = flOUT_class_3C.rsplit("_",1)[0] + '_Detections.csv'
    ...
    if hd_3C != None: # This condition adds a header to the new data.
        wrtrC_3C.writerow(hd_3C)
        wrtrD_3C.writerow(hdD)
        hd_3C = None	# for clasification
