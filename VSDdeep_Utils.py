import os, shutil, h5py, time, multiprocessing
import pandas as pd
import numpy as np
...
def _block_CNN_1(filters, ker, drop_rate, activation, padding, inpC): 
    ...
    return res_out  
def _encoder(filter_number, filter_size, decimations, depth, drop_rate, ker_regul, bias_regul, activation, padding, inpC):
    ...
    for dp in range(depth):
        ...
        e = MaxPooling1D(decimations[dp], padding = padding)(e)            
    return(e)

def _decoder(filter_number, filter_size, decimations, depth, drop_rate, ker_regul, bias_regul, activation, padding, inpC):
    ' Returns the dencoder that is a combination of residual blocks and upsampling. '           
    d = inpC
    for dp in range(depth):        
        d = UpSampling1D(decimations[dp])(d) 
        if dp == 3:
...
class cred2():
    ...
    def __call__(self, inp):
        ...
        decoder_D = _decoder([i for i in reversed(self.nb_filters)], 
                             ...
                             [i for i in reversed(self.decimations)], 
                             self.decoder_depth, 
                             self.drop_rate, 
                             self.kernel_regularizer, 
                             self.bias_regularizer,
